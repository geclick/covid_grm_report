...
class MultipleSerializerMixin(object):
    ...
    def get_serializer_class(self):
        serializer_class = (
            self.get_serializer_class_by_action() or self.serializer_class
        )

        assert serializer_class is not None, (
            "'%s' should either include one of `serializer_class` and `serializer_classes` attribute, "  # noqa: E501
...
class BaseNestedModelSerializer(serializers.ModelSerializer):
    ...
    def _get_generic_lookup(self, instance, related_field):
        return {
            related_field.content_type_field_name: ContentType.objects.get_for_model(  # noqa: E501
                instance
            ),
...
class NestedUpdateMixin(BaseNestedModelSerializer):
    ...
    default_error_messages = {
        "cannot_delete_protected": _(
            "Cannot delete {instances} because " "protected relation exists"
        )
    }
...
class UniqueFieldsMixin(serializers.ModelSerializer):
    ...
    def _validate_unique_fields(self, validated_data):
        for field_name in self._unique_fields:
            ...
            try:
                # `set_context` removed on DRF >= 3.11,
                # pass in via __call__ instead
